...
using System.Windows.Forms; // MessageBox로 에러메세지를 출력해주기 위해서 추가한다.
    ...
    public static class Helper
    {
        ...
        public static List<T> DataReaderMapToList<T>(IDataReader dr)
        {
            ...
            catch (Exception err)
            {
                ...
                MessageBox.Show(msg, "에러 메세지 확인"); // MessageBox로 에러메세지를 출력해주기 위해서 추가한다.
                return null;
            }
        }
        ...
        private static bool ContainsColumn(IDataReader reader, string columnName)
        {
            foreach (DataRow row in reader.GetSchemaTable().Rows)
            {
                if (row["ColumnName"].ToString() == columnName)
                    //if (row["ColumnName"].ToString().Equals(columnName, StringComparison.OrdinalIgnoreCase); ==> 쿼리에서의 대소문자 구분없이 사용하기 위한 코드
                    return true;
            }  
            return false;
        }
